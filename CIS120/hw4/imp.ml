...
let rec assoc (k: 'k) (l: ('k * 'v) list) : 'v option =
  begin
    match l with
    | []         -> None
    | (hk,hv)::l -> if k=hk then Some hv else assoc k l
  end
...
let rec partial_transform (f : 'a -> 'b option) (x : 'a list) : 'b list =
  begin
    ...
    | h::t ->  begin
                 match f h with
                 | None   -> partial_transform f t
                 | Some x -> x::(partial_transform  f t)
               end
  end
...
let rec contains_alias_option (x: 'a) (l: 'a option list) : bool =
 begin
   ...
   | h::t  -> begin
               match h with
               | None   -> false
               | Some v -> if v==x then true else contains_alias_option x t 
              end
 end
