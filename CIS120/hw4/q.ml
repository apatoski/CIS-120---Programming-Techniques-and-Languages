...
let contains (elt: 'a) (q: 'a queue) : bool =
  ...
  let rec loop (no: 'a qnode option) =
   begin
    match no with
    | None   -> false
    | Some v -> (if v.v == elt then true else false) || loop v.next
   end
	in loop q.head
...
(* Implement a function that truncates a queue after the first
   ...
   We will be manually grading the tests for truncate. *)
  ...
  if not (valid q) then failwith "truncate: given invalid queue"
   ...
   let rec loop (no: 'a qnode option) =
    begin
     match no with
     | None   ->  ()
     | Some v -> if v.v == elt then (v.next<-None; q.tail<- Some v) 
                else loop v.next
    end
   in loop q.head
...
let test () : bool =
	let q=from_list [1;1;1;22;3] in
  truncate 1 q; print q string_of_int ; q=from_list[1]
;; run_test "test" test
...
(* Implement a function that deletes all nodes containing the value v
   ...
   Note: your TAs will be grading your test cases for this problem. 
     You should add tests, based on the situations above, at the 
...
let delete (elt: 'a) (q: 'a queue) : unit =
  if not (valid q) then failwith "delete: given invalid queue"
   ...
   let rec loop (elt:'a) (prev:'a qnode option) (no:'a qnode option)=
    begin
     match (prev,no) with
		 | (_, None)      -> q.tail<-prev
     | (None, Some x) -> if x.v==elt then 
                         (q.head<-x.next; loop elt None x.next)
                         else loop elt no x.next
     | (_, Some x)    -> if x.v==elt then 
                         ((ext prev).next<-x.next; loop elt prev x.next)
                         else loop elt no x.next
    end
	in loop elt None q.head
...
let test () : bool =
  let q1 = from_list [5;5;3] in
  truncate 5 q1; to_list q1= [5]
;; run_test "two same at head" test

let test () : bool =
  let q1 = from_list [1;2;5;3] in
  truncate 5 q1; to_list q1=  [1;2;5]
;; run_test "mid" test

let test () : bool =
  let q1 = from_list [] in
  truncate 5 q1; to_list q1=  []
;; run_test "empty" test

let test () : bool =
  let q1 = from_list [5] in
  truncate 5 q1; to_list q1= [5]
;; run_test "singleton" test

let test () : bool =
  let q1 = from_list [1;2;3;5;5] in
  truncate 5 q1; to_list q1= [1;2;3;5]
;; run_test "two same at tail" test

let test () : bool =
  let q1 = from_list [5;5;3] in
  truncate 3 q1; to_list q1=  [5;5;3]
;; run_test "trunc at tail" test

let test () : bool =
  let q1 = from_list [5;5;3] in
  truncate 1 q1; to_list q1=[5;5;3]
;; run_test "no elem" test


(*-------------*)
let test () : bool =
  let q1 = from_list [5] in
  delete 5 q1; to_list q1=  []
;; run_test "singleton" test
...
let test () : bool =
  let q1 = from_list [5;3;5] in
  delete 5 q1; to_list q1= [3]
;; run_test "head and tail" test

let test () : bool =
  let q1 = from_list [5;5;5] in
  delete 5 q1; to_list q1= []
;; run_test "all bad" test

let test () : bool =
  let q1 = from_list [5] in
  delete 1 q1; to_list q1= [5]
;; run_test "no elem" test

let test () : bool =
  let q1 = from_list [1;5;3] in
  delete 5 q1; to_list q1= [1;3]
;; run_test "average case" test
