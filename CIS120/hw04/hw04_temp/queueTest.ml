...
(* `QueueTest` is a reuseable module that we'll use to test other
...
module QueueTest (QImpl: QueueInterface.QUEUE) = struct
   ...
   * they prepend the name of the implementation we're testing to the test
   * description. *)
   ...
   * the test below will fail for queues whose `from_list` function is not yet
   * implemented (even if `is_empty` is correct).  But this is fine: you can
   * write the tests now (and make sure that they typecheck and compile), then
   * fill in the functions and use the tests you've written to help debug
   * them.  Of course, you can add more tests later, if implementing 
   * the functions makes you realize you forgot some cases.  But you'll
   * find the assignment easiest if you try to write a good set of tests 
   * first, before implementing any of the functions. *)
   ...
   * functions defined in the `QUEUE` interface. It will probably be helpful to
   * have the file queueInterface.ml open as you work. Your tests should
   * stress the abstract properties of what it means to be a queue, as well
   * as the relationships among the operations provided by the QUEUE interface.
   *
   * You do not need to test the `print` function.
   *
   * We provide many test cases for you, so your job here is to finish writing
   * tests for `truncate` and `delete`.
   *
