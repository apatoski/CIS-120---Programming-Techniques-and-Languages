...
public class AdvancedManipulations {
	...
	public static PixelPicture adjustContrast(
	       ...
	       for (int x = 0; x < w; x++) {
	           for (int y = 0; y < h; y++) {
	              avg+= src [x][y].getRed() + src [x][y].getGreen() + src[x][y].getBlue();
	           }
	       }
	       avg=(int)(Math.round(avg/(3.0*w*h)));
	       for (int x = 0; x < w; x++) {
	           for (int y = 0; y < h; y++) {
	              r=(int) (multiplier*(src[x][y].getRed()-avg)+avg);
	              g=(int) (multiplier*(src[x][y].getGreen()-avg)+avg);
	              b=(int) (multiplier*(src[x][y].getBlue()-avg)+avg);
	...
	public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
		...
		for (int x=0;x<w;x++)
			for(int y=0;y<h;y++)
			{
				if (colors.contains(src[x][y]))
				colors.put(src[x][y], colors.getValue(src[x][y])+1);
				else colors.put(src[x][y], 0);
			}
		Pixel [] freq= colors.getSortedPixels();
		int minDist; int minIndex; int helper;
		for (int x=0;x<w;x++)
			for(int y=0;y<h;y++)
			{ 
				minDist = src[x][y].distance(freq[0]);
				minIndex= 0;
				for(int i=1;i<numColors;i++)
				{
					helper =src[x][y].distance(freq[i]); 
					if(helper<minDist) 
					{
						minDist = helper; 
						minIndex = i;
	 ...
	 */
	private static boolean ok (int x,int y, int w,int h)
	{
	...
	public static PixelPicture blur(PixelPicture pic, int radius) {
	    ...
	    int h = pic.getHeight();
	    int r,g,b,num;
	    Pixel hh=new Pixel(0,0,0);
        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[w][h];
        for (int x=0;x<w;x++)
        	 for (int y=0;y<h;y++)
        	 {   
        		 r=0;g=0;b=0;num=0; hh=null;
        		 for (int i = x - radius; i <= x + radius; i++)
        		 {
        			 for (int j = y - radius; j <= y + radius; j++)
            		 {
            			 if (ok(i,j,w,h))
            			 {
            				 num++; hh=src[i][j];r+=hh.getRed(); g+=hh.getGreen(); b+=hh.getBlue();
            			 }
            		 }
        		 }
        		 tgt[x][y]=new Pixel((1.*r)/num,(1.*g)/num,(1.*b)/num);
        	 }
