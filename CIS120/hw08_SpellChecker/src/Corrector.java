...
/**
 * Skeleton implementation for classes capable of providing spelling
 * corrections.
 * <p>
 * The matchCase method is code that is shared among all Correctors.
 * <p>
 * Concrete subclasses of this abstract class should implement the
 * getCorrections method (which will usually call matchCase).
 */
public abstract class Corrector {
	 ...
	 *
	 * @param incorrectWord
	 *            the word whose case should be matched
	 * @param corrections
	 *            the set whose capitalization should be fixed
	...
	public Set<String> matchCase(String incorrectWord, Set<String> corrections) {
		...
		for (String s : corrections) {
			if (capitalizeFirst) {
				String ucfirst = s.substring(0, 1).toUpperCase() + s.substring(1).toLowerCase();
	 ...
	 * The corrections should match the case of the input; the matchCase method
	 * is helpful here.
	 * <p>
	 * Any input that is *not* a valid word (i.e. only composed of letters
	 * and/or apostrophes) should throw an IllegalArgumentException.
	 *
	 * @param wrong
	 *            the misspelled word
