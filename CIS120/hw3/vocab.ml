 ...
 * We can use the functions from `HigherOrder` and the `SET` interface to do so.
 * (Note that because we haven't explicitly opened HigherOrder, you need to put
 * `HigherOrder.` before the function names if you'd like to use them). *)

let texts: (string * string set) list =
  HigherOrder.transform (fun (name, l) -> (name, set_of_list l)) texts_as_lists
 ...
 * list that match the given predicate. Hint: Functions from higherOrder.ml may
 * come in handy here!. *)

let count_if (pred: 'a -> bool) (l: 'a list) : int =
  fold (fun x acc -> (if pred x then 1 else 0) + acc) 0 l

(* Use the `count_if` function defined above to determine the number of words in
 * the vocab list that appear in a given text (represented as a set). *)
...
let count_sat_vocab_starting_with (c:char) : string set -> int =
 let c_sat_vocab = filter (starts_with c) sat_vocab in
 count_words_in_text c_sat_vocab
...
let frequency_table: (char * int list) list =
  let outer = transform count_sat_vocab_starting_with abcs in  (* list partial f-s *)
  let trimmed_txts = transform (fun (x,y) -> y) texts in 
  let rec unlabeled_table (x: (string set ->int) list)=
		begin match x with
     | []    -> []
     | h::t -> (transform h trimmed_txts)::(unlabeled_table t)
		end
	in
	let rec label (x: char list) (l: int list list): (char * int list) list =
		begin
			match x,l with
			| ([a],[b]) -> [(a,b)]
			| (a::t1,b::t2)   -> (a,b) :: (label t1 t2)
		end
	in
	label abcs ( unlabeled_table outer)
...
let table_as_string: string =
  let header = "   " ^
    (String.concat "" (HigherOrder.transform (fun (name, _) -> name) texts))
    ^ "\n"
  in
  let row (c, l) =
    (Char.escaped c) ^ ": " ^
    (String.concat "      " (HigherOrder.transform string_of_int l))
  in
  header ^
  (String.concat "\n" (HigherOrder.transform row frequency_table))
