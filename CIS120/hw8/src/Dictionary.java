 ...
 *
 * The dictionary is case insensitive and only stores "valid" word. A valid word
 * is any sequence of letters (as determined by Character.isLetter) or
 * apostrophes characters.
 */
public class Dictionary {
	 ...
	 * characters. All non-word tokens provided by the TokenScanner should be
	 * ignored.
	 *
	 * <p>
	 *
	 * @param ts
	 *            sequence of words to use as a dictionary
	 * @throws IOException
	...
	public Dictionary(TokenScanner ts) throws IOException {
		...
		while (true) {
			if (ts.hasNext() == false) {
				...
				String curr = ts.next();
				if (TokenScanner.isWord(curr)) {
					dictionary.add(curr.toLowerCase());
				}
			}
		}
	 ...
	 *
	 * @param filename
	 *            location of file to read words from
	...
	/**
	 * Returns the number of unique words in this dictionary. This count is case
	 * insensitive: if both "DOGS" and "dogs" appeared in the file, it should
	 * only be counted once in the returned sum.
	 * 
	...
	/**
	 * Test whether the input word is present in the Dictionary. If the word is
	 * not in the Dictionary the method should return false. Note that only
	 ...
	 * <p>
	 * This check should be case insensitive. For example, if the Dictionary
	 * contains "dog" or "dOg" then isWord("DOG") should return true.
	 * <p>
